...
def add_view(vin: str, session: database.Session = Depends(database.get_session)):
    ...
    if len(items) >= 1:
        ...
        query = session.query(CarVc).filter(CarVc.vin == vin.upper(), CarVc.is_hidden == False)
        ...
        if len(items) == 0:
            ...
            session.query(CarVc).filter(CarVc.vin == vin.upper()).update({CarVc.views: CarVc.views+1})
            #carvc = session.query(CarVc).filter(CarVc.vin == vin.upper())
            #setattr(carvc, 'views', CarVc.views + 1)
            session.commit()
            #session.flush()
            #session.commit()
            end = time.time()
